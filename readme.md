...
Это конечно не очень хорошо, так как я собирался использовать для доступа к ним мою [обертку над регистрами](https://habr.com/ru/post/459642/), а она не подходит для доступа к регистрам ядра, из-за того, что доступ к регистрам ядра осуществляется особым образом через эти специальные команды. 

Чтобы не трогать уже написанную обертку и генератор, я сделал отдельный класс для их обработки. 
...
|0xF11|MRO|mvendorid|(Machine Vendor ID Register) Регистр содержащий код провайдера ядра, который выдается JEDEC ассоциацией
|0xF12|MRO|marchid|(Machine Microacrhitecture ID Register) Идентификатор микроархитектуры ядра
|0xF13|MRO|mimpid|(Machine Implementation ID Register) Идентификатор номера версии реализации ядра.
...
|0x305|MRW|mtvec|(Machine Trap-Vector Base-Address Register) Регистр содержит конфигурацию обработчика(ловушки) исключений, которая состоит из адреса этого обработчика.
...
Указатели на функции для обработки прерываний положим в одномерный массив
...
Точно такой же "фокус" провернем с обработкой исключений. Только таблица будет поменьше, исключений всего 12
...
```
 ...
 Функция main выглядит очень скромно
 ```cpp
int main()
{
  while (true)
