...
#include "corefieldvalue.hpp"      //for BitsFileds
...
class CoreRegister
{
 public:
    ...
    static void Set()
    {
        Type newRegValue = __read_csr(address);  //Сохраняем текущее значение регистра
        ...
        __write_csr(address, newRegValue); // Записываем в регистра новое значение
    }

    __forceinline template<typename T = AccessMode,
        class = typename std::enable_if_t<std::is_base_of<WriteMode, T>::value ||
                                          std::is_base_of<ReadWriteMode, T>::value>>
    static void SetValueBits()
    {
        static constexpr auto value = GetValue();
        __set_bits_csr(address, value); // Записываем в регистра новое значение
    }
    ...
    {
        static constexpr auto value = GetValue();
        __write_csr(address, value); //Записываем в регистра новое значение
    }
